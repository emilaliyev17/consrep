...
interface ConsolidatedBSProps {
  data: ConsolidatedRow[];
  periodNames: string[];
  companyNames: string[];
}

const ConsolidatedBS: React.FC<ConsolidatedBSProps> = ({ data, periodNames, companyNames }) => {
  ...
  if (!data || data.length === 0) {
    return <Typography>No Balance Sheet data to display.</Typography>;
  }
  ...
  return (
    <>
      ...
      <TableContainer component={Paper} sx={{ maxHeight: 'calc(100vh - 200px)', width: '100%', overflowX: 'auto', border: '1px solid #ddd' }}>
        <Typography variant="h5" sx={{ p: 2 }}>Consolidated Balance Sheet</Typography>
